...
const go = async () => {
  ...
  await getFileAndStoreLocally(
    ...
    (text) => {
      const renameImport = text.replace(
        'typescriptlang-org/static/js/sandbox',
        '../sandbox',
      );
      return renameImport;
    },
  ...
  await getFileAndStoreLocally(
    ...
    (text) => {
      ...
      const removedLZ = removeImports.replace(
        'import("lz-string").LZStringStatic',
        'any',
      );
      return removedLZ;
    },
  ...
  await getFileAndStoreLocally(
    ...
    (text) => {
      ...
      const addedTsWorkerImport =
        'import { TypeScriptWorker } from "./tsWorker";' + removedLZ;
  ...
  await getFileAndStoreLocally(
    ...
    (text) => {
      const replaceSandbox = text.replace(
        '"@typescript/sandbox"',
        '"./sandbox"',
      );
